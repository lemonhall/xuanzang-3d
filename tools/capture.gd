...
func _capture() -> void:
	...
	for i in range(args.size()):
		if args[i] == "--out" and i + 1 < args.size():
			...
			# 幻影原点在城中心，像身在本地的 (-800, 0, -1500)，
			# 所以节点摆到 (3300, 0, 1500) 时，像身正好落在正前方 2500 m。
			# （朝向不用动：节点 z=+1500 已经抵消了像身本地的 z=-1500。）
			_hold_position = Vector3(3300.0, 0.0, 1500.0)
			_pitch_deg = 22.0
	...
	# 头几十帧用来生成地形、编译着色器、让雾和光照稳定下来
	for i in range(frames):
		await get_tree().process_frame
		if _hold_position != Vector3.INF:
			var held: Node3D = _main.get("mirage")
			if held != null:
				held.global_position = _hold_position
	...
	if err == OK:
		...
		print("CAPTURE FAILED (%d): %s" % [err, out_path])
...
func _diagnose_landmarks(cam: Camera3D, mirage: Node3D) -> void:
	...
	var points := {
		...
		"弥勒头": Vector3(statue_x, o.y + Mirage.STATUE_HEIGHT * 0.902, statue_z),
		"弥勒肩": Vector3(statue_x, o.y + Mirage.STATUE_HEIGHT * 0.788, statue_z),
		"弥勒脚": Vector3(statue_x, o.y, statue_z),
		"举身光顶": Vector3(
			statue_x + Mirage.STATUE_HEIGHT * 0.140,
			o.y + Mirage.STATUE_HEIGHT * 1.22,
			statue_z
		),
	}
...
func _swap_mirage_material() -> void:
	var mirage: Node3D = _main.get("mirage")
	if mirage == null:
		return
	var instance: MeshInstance3D = mirage.get_node_or_null("Loulan")
	if instance == null:
		print("DIAG: Loulan node missing")
		return
	...
	print(
		"DIAG: swapped to plain magenta; surfaces=%d aabb=%s"
		% [instance.mesh.get_surface_count(), instance.get_aabb()]
	)
