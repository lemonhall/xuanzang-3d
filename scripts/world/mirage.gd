...
##   正面城墙 1900 m 外、400 m 高 → 12.1°（贴着地平线站起来的一条横带）
##   城门楼   1900 m 外、620 m 高、方位 +23.6°（画面右侧的第二个高点）
##   主塔     2900 m 外、1500 m 高、方位 +16.0°（城郭的最高点）
##   角楼     方位 -52.8°（左）/ +65°（右）——两端都在画外，转头的瞬间才进场
## 注意后三个方位是**偏右**的：城郭整体右移了 CITY_SIDE，让开被弥勒像挡住
## 的那根轴线（见 CITY_SIDE 的注释）。城不对称，是这一版的**有意为之**。
...
##
## **城郭是不对称的**，两个端头各有各的职责，别把它们改回对称：
##
##   * 左端（玩家左手那一端）钉死在 `CITY_LEFT_END`——它守的是"横向量不出来"：
##     只要城墙的一端还留在画面里，大脑就会量出它的尺寸，那它就只是
##     "一座大一点的城"。这一条是硬约束，**不许往右挪**。
##   * 右端随 `CITY_SIDE` 一起往右长——它让出的是**地标**（见 CITY_SIDE）。
const CITY_LEFT_END := -2500.0
## 城郭整体向右（+Z）挪多少米。
##
## 为什么要挪：弥勒像站在城墙之前 480 m、横向偏到玩家左手边 700 m，从玩家
## 看过去它占了画面左半边（像的剪影右缘落在方位 0° 附近）。城郭原来的中轴
## 在方位 0°——**主塔和城门楼正好贴着像的右缘立起来**，被像挡掉一半。
## 需求方 2026-09-24 的红框截图就是这么说的："那个佛陀背后的城市啊，你向右
## 移动一些……否则佛会把它挡住。"
##
## 800 m 把主塔推到方位 +16°（左缘 +10°）、城门楼推到 +23°：
## 地标整根从像的轮廓里让出来，中间还留 8° 以上的天。
## 挪的是**城**，不是像——像和太阳一动不动（太阳是按像的方位反解出来的，
## 见 main._place_sun_behind_statue）。像背后仍然压着城墙：城墙很长，
## 左端还在 -2500 m。
const CITY_SIDE := 800.0
## 主塔（城郭里最靠左的地标）的左缘与弥勒像剪影右缘之间必须留出的天空间隙。
##
## "佛塔不能被佛挡住"这句话只有落成一个**角度**才有可能守住：两尊巨物都是
## 剪影，谁都看不出差了两度还是二十度，测试却看得出。8° 在 1600 px 宽的
## 水平前视里约 130 px——足够读成"两件事"，又不至于把城推出画面。
##
## 量的不是包围盒：像身最宽处在 0.55h（举臂那一层），而它已经在画面上沿
## 之外；用包围盒量会得到 +31°，把整座城都逼出画面。要量的是**画面上确实
## 看得见的那一段**，见 tests/test_scene.gd 里 `[幻影不是穹顶]` 的第 4 条。
const CITY_CLEARANCE_DEG := 8.0
## 城郭半宽 = 左端到城心（= CITY_SIDE）的距离。
##
## 2500 m 那一半是被**观距**反推出来的，不是拍脑袋：城墙在 1900 m 外时，
## 2200 m 的半宽只剩 ±49.2°，而水平半视角是 50.2°——两端会露进画面。
## 2500 m 给出 ±52.8°，留 2.6° 余量。改城墙距离必须同时改这个数。
const CITY_HALF := CITY_SIDE - CITY_LEFT_END
const WALL_H := 400.0
const WALL_T := 260.0
...
func _design() -> void:
	...
	# 横向的 CITY_SIDE 和城郭是同一个偏移：塔在城里，不是城旁边。
	_bucket = BUCKET_CITY
	_part_scale = TOWER_SCALE
	_part_offset = Vector3(-CITY_HALF + TOWER_BEHIND_WALL, 0.0, CITY_SIDE)
	_design_terraces()
	_design_parapet()
	...
	# 城郭：真实米，1:1
	# 整体右移 CITY_SIDE：让出被弥勒像挡住的那根轴线（见 CITY_SIDE 的注释）。
	# 这个偏移**不能**挪到 Mirage 节点上去——节点一动，像和太阳跟着动，
	# 逆光的构图就全变了。要挪的只是城。
	_bucket = BUCKET_CITY
	_part_offset = Vector3(0.0, 0.0, CITY_SIDE)
	_design_city()
...
## 像身在**世界坐标**里的基点（白膜原点 = 台座底面，在沙面以下 statue_sink()）。
##
## 玩家这边要"朝着佛伸手"，就得有一个世界坐标可以去指。用 `global_position +
## statue_offset()` 而不是在别处把 CITY_HALF / STATUE_SIDE 再抄一遍：像挪了，
## 手也跟着挪，不需要谁记得去改第二个地方。
func statue_world_position() -> Vector3:
	return global_position + statue_offset()


## 伸手/抬头要指的那个点：像身自上而下 `ratio` 处（0 = 沙面，1 = 像头）。
##
## 不指脚下的沙、也不指头顶的天：0.45 落在**佛的胸怀一段**——手伸出去时，
## 画面里是"手臂斜向上指向巨像的躯干"，而不是"手指着地平线上一个小点"。
func statue_focus_point(ratio := 0.45) -> Vector3:
	return statue_world_position() + Vector3(0.0, statue_top_y() * ratio, 0.0)
